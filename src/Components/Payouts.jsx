  ...
  return (
    <div className="panel" style={{ background: "rgba(19, 31, 56, 0.7)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "12px", padding: "24px" }}>
      <div className="panel-head" style={{ marginBottom: "20px" }}>
        ...
        <p style={{ color: "#8a99ad", margin: "4px 0 0 0", fontSize: "14px" }}>Manage partner payments, custom platform cuts, and standard transaction clearances.</p>
      </div>
            ...
            {payouts.map((p) => (
              <tr key={p.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)", fontSize: "15px" }}>
                ...
                <td style={{ padding: "16px 12px", textAlign: "right" }}>
                  {p.status === "ready" ? (
                    <button onClick={() => triggerPayout(p.id)} style={{ background: "#3b82f6", color: "#fff", border: "none", padding: "6px 12px", borderRadius: "6px", cursor: "pointer", fontSize: "12px", fontWeight: "500" }}>
                      💸 Release Fund
                    </button>
