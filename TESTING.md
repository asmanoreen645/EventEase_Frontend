...
| 2026-09-10 | Services Page - Mobile Responsive View | Mobile view (600px width) mein Services page check kiya | Layout sahi adjust hona chahiye | Sab sections (Hero, Categories, How It Works, Featured, Stats, FAQ, CTA) sahi responsive the | ✅ Pass |
...
| 2026-09-10 | Vendors Page - Mobile Responsive View | Mobile view (600px width) mein Vendors page check kiya | Layout sahi adjust hona chahiye, Filters sidebar sahi dikhni chahiye | Sab kuch sahi responsive tha, Filters sidebar bhi sahi adjust hui | ✅ Pass |
| 2026-09-10 | Sign Up Page - Mobile Responsive View | Mobile view mein Sign Up form check kiya | Layout sahi adjust hona chahiye, fields fit honi chahiye | Layout sahi responsive tha, fields sahi fit hui. Registration fail hui (same known backend issue - MongoDB not connected) | ✅ Pass (Layout) - Known Issue confirmed on mobile too |
...
| 2026-09-10 | Login Page - Mobile Responsive View | Mobile view mein Login form check kiya | Layout sahi adjust hona chahiye, fields fit honi chahiye | Layout sahi responsive tha, fields sahi fit hui. Login fail hua (same known backend issue - MongoDB not connected) | ✅ Pass (Layout) - Known Issue confirmed on mobile too |
...
| 2026-09-14 | Vendor Profile - Chat with Vendor (own profile) | Logged in as vendor, viewed own "My Profile" page, clicked "Chat with Vendor" | Since this is the vendor's own profile, this button should not be shown, or should be disabled | Clicked button attempted to open chat with self, showing error "Koi active vendor available nahi hai chat ke liye" | ❌ Fail (Logic bug - own profile shouldn't show Chat/Book buttons) |
| 2026-09-14 | Chat Feature - Customer to Vendor | Logged in as customer on separate device, clicked "Chat with Vendor" on a real vendor's (Mahi Photographer) profile | Chat window should open and allow messaging | Page stuck on "Loading real vendor..." — chat never opened | ❌ Fail (Possibly related to Render backend performance issue, needs investigation once Render access available) |
| 2026-09-14 | Booking Page - Input Fields Text Visibility (Retest after fix) | Typed text in booking form input fields after fix | Typed text should be clearly visible | Text is now clearly visible after adding `color: #111827;` to BookingDetails.css | ✅ Pass (Fixed) |
## Fixes Applied
...
- Vendor's own "My Profile" page still shows "Chat with Vendor" and "Book Now" buttons, which don't make sense for a vendor viewing their own profile. These buttons should be hidden when the logged-in user is viewing their own vendor profile.